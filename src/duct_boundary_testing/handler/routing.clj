(ns duct-boundary-testing.handler.routing
  (:use [hiccup.core]
  ...
  (:require
            ...
            [duct-boundary-testing.boundary.users :as db.users]
            [duct-boundary-testing.spec.user :as s.user]
...
(defn edit-user-view [user user-id error-messages]
  (html [:div "[ Edit User ]"
...
;;    (edit-user-view user id nil)))

(defn edit [db id]
  (-> (db.users/get-user db id)
      first
      (edit-user-view id nil)))

(defn del [db id]
  (if (= '(1) (db.users/delete-user db ""))
    (redirect "/users")
;;    (let [user (first (db.users/get-user db id))]
;;      (show-user-view user))))
    (->> (db.users/get-user db id)
...
;;      (redirect (str "/users/" (:generated_key rslt))))
    (->> (db.users/create-user db params)
         first
         :generated_key
         (str "/users/")
         redirect)
...
(defn upd [db id {:keys [params]}]
  (if (and
    ...
    (edit-user-view params id (s.user/error-messages params))))

;;============================================================

;;(defmethod ig/init-key :duct-boundary-testing.handler/routing [_ {:keys [db]}]
(defmethod ig/init-key :duct-boundary-testing.handler/routing [_ duct-db]
;;(defmethod ig/init :duct-boundary-testing.handler/routing [_ duct-db]
